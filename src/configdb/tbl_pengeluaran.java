/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package configdb;
import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.Driver;
import java.sql.Statement;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import javax.swing.JOptionPane;
import javax.swing.JTable;
import java.sql.ResultSetMetaData;
import javax.swing.table.DefaultTableModel;
/**
 *
 * @author BAGUS NUR PRATAMA
 */
public class tbl_pengeluaran {
    private String namadb = "pbo2_2310010457";
    private String url = "jdbc:mysql://localhost:3306/" + namadb;
    private String username = "root";
    private String password = "";
    private Connection koneksi; 
    public Integer VAR_idpetugas = null;
    public Integer VAR_idkategori = null;
    public String VAR_tglpengeluaran = null;    
    public String VAR_nominal = null;
    public String VAR_keterangan = null;
    public boolean validasi = false;
    
    public tbl_pengeluaran(){
        try {
            Driver mysqlDriver = new com.mysql.cj.jdbc.Driver();
            DriverManager.registerDriver(mysqlDriver);
            koneksi = DriverManager.getConnection(url, username, password);
            System.out.println("Berhasil dikoneksikan ke database");
        } catch (SQLException error) {
            JOptionPane.showMessageDialog(null, error.getMessage());
        }
    }    
    
    public void SimpanAnggota(Integer ID1, Integer idpetugas, Integer idkategori, String tgl, String nominal,
                               String ket){
        try {
            // gunakan nama tabel yang benar
            String sql = "INSERT INTO tbl_pengeluaran(id_pengeluaran, id_petugas, id_kategori, tgl_pengeluaran, nominal, keterangan) "
                       + "VALUES('"+ID1+"', '"+idpetugas+"', '"+idkategori+"', '"+tgl+"', '"+nominal+"', '"+ket+"')";

            // cek apakah id sudah ada
            String cekPrimary = "SELECT * FROM tbl_pengeluaran WHERE id_pengeluaran = '"+ID1+"'";

            Statement check = koneksi.createStatement();
            ResultSet data = check.executeQuery(cekPrimary);

            if (data.next()) {
                JOptionPane.showMessageDialog(null, "ID petugas sudah terdaftar!");                
                this.VAR_idpetugas = data.getInt("id_petugas");
                this.VAR_idkategori = data.getInt("id_kategori");                
                this.VAR_tglpengeluaran = data.getString("tgl_pengeluaran");
                this.VAR_nominal = data.getString("nominal");
                this.VAR_keterangan = data.getString("keterangan");
                this.validasi = true;
            } else {
                this.validasi = false;
                Statement perintah = koneksi.createStatement();
                perintah.executeUpdate(sql);
                JOptionPane.showMessageDialog(null, "Data berhasil disimpan!");
            }

        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Terjadi kesalahan: " + e.getMessage());
            e.printStackTrace();
        }
    }    
    
        public void ubahAnggota(Integer ID1, Integer idpetugas, Integer idkategori, String tgl, String nominal,
                               String ket){
        try {
            // nama tabel dan kolom disesuaikan
            String sql = "UPDATE tbl_pengeluaran SET id_petugas=?, id_kategori=?, tgl_pengeluaran=?, "
                    + "nominal=?, keterangan=? WHERE id_pengeluaran=?";

            PreparedStatement perintah = koneksi.prepareStatement(sql);
            perintah.setInt(1, idpetugas);
            perintah.setInt(2, idkategori);
            perintah.setString(3, tgl);
            perintah.setString(4, nominal);
            perintah.setString(5, ket);                       
            perintah.setInt(6, ID1);
            perintah.executeUpdate();

            JOptionPane.showMessageDialog(null, "Data berhasil diubah!");
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Terjadi kesalahan: " + e.getMessage());
            e.printStackTrace();
        }
    } 
        
    public void hapusAnggota01(Integer ID1){
        try {
            // hapus dari tabel yang benar
            String sql = "DELETE FROM tbl_pengeluaran WHERE id_pengeluaran='"+ID1+"'";
            Statement perintah = koneksi.createStatement();
            perintah.execute(sql);
            JOptionPane.showMessageDialog(null, "Data berhasil dihapus!");
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Terjadi kesalahan: " + e.getMessage());
            e.printStackTrace();
        }
    }    
    
        public void tampilDataAgenda(JTable komponenTable, String SQL){
            try {
              Statement perintah = koneksi.createStatement();
              ResultSet data = perintah.executeQuery(SQL);
              ResultSetMetaData meta = data.getMetaData();
              int jumKolom = meta.getColumnCount();
              DefaultTableModel modelTable = new DefaultTableModel();
              modelTable.addColumn("id_pengeluaran");
              modelTable.addColumn("id_petugas");
              modelTable.addColumn("id_kategori");
              modelTable.addColumn("tgl_pengeluaran");
              modelTable.addColumn("nominal");
              modelTable.addColumn("keterangan");
              modelTable.getDataVector().clear();
              modelTable.fireTableDataChanged();
              while (data.next() ) {
                  Object[] row = new Object[jumKolom];
                  for(int i = 1; i <= jumKolom; i++ ){
                      row [i - 1] = data.getObject(i);
                  }
                  modelTable.addRow(row);
              }
              komponenTable.setModel(modelTable);
          } catch (Exception e) {
              
          }
        }    
    
    
    
    
}
